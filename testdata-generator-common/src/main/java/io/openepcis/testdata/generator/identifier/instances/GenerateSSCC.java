...
public class GenerateSSCC extends GenerateEPCType2 {
  ...
  // Generate the SSCC identifiers based on the serialType and format them according to URN or
  // WebURI format.
  private List<String> generateIdentifiers(
    ...
    try {
      ...
      final String prefix =
          syntax.equals(IdentifierVocabularyType.URN) ? SSCC_URN_PART : dlURL + SSCC_URI_PART;
      ...
      if (SerialTypeChecker.isRangeType(this.serialType, count, this.rangeFrom)) {
        // Generate SEQUENTIAL/RANGE SSCC identifiers in URN/WEBURI format based on from value and
        // count
        for (var rangeID = rangeFrom.longValue();
            rangeID < rangeFrom.longValue() + count;
            rangeID++) {
          formattedSSCC.add(
              prefix + modifiedGCP + suffix + padAndAppend(modifiedGCP, rangeID, appendLength));
        }
        ...
        formattedSSCC.add(
            prefix + modifiedGCP + suffix + padAndAppend(modifiedGCP, serialNumber, appendLength));
      }
      return formattedSSCC;
    } catch (Exception ex) {
      throw new TestDataGeneratorException(
          "Exception occurred during generation of SSCC instance identifiers : " + ex.getMessage(),
          ex);
    }
  }
  ...
  private String padAndAppend(final String baseGcp, final Object id, final int totalLength) {
    return StringUtils.repeat("0", totalLength - (baseGcp.length() + String.valueOf(id).length()))
        + id;
  }
}
