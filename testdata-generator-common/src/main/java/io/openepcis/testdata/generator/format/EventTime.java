...
public class EventTime implements Serializable {
  ...
  @Schema(
      ...
      description = "Custom offset From value for generating eventTime based on parent eventTime")
  private Integer customOffsetFrom;

  @Schema(
      type = SchemaType.INTEGER,
      description = "Custom offset To value for generating eventTime based on parent eventTime")
  private Integer customOffsetTo;

  @Schema(
      type = SchemaType.STRING,
      description =
          "Custom offset type value for generating eventTime based on parent eventTime ex: seconds/minutes/hours/days")
  private String customOffsetType;

  public OffsetDateTime generate(
      final OffsetDateTime parentEventTime,
      final RandomSerialNumberGenerator randomDateTimeGenerator) {
    try {
      if (specificTime != null) {
        ...
        return Instant.ofEpochMilli(
                randomDateTimeGenerator.generateIntInRange(
                    fromTime.toInstant().toEpochMilli(), toTime.toInstant().toEpochMilli()))
            .atZone(ZoneId.systemDefault())
            .toOffsetDateTime();
      } else if (parentEventTime != null
          && customOffsetFrom != null
          && customOffsetTo != null
          && StringUtils.isNotBlank(customOffsetType)) {
        // For custom offset from and to date time get the value based on the parentEventTime if
        // parentEventTime is not null
      ...
      throw new TestDataGeneratorException(
          "Exception occurred during creation of assignment of the Event Time : " + ex.getMessage(),
          ex);
    }
  }

  // Method to calculate the eventTime based on custom offset values
  private OffsetDateTime calculateCustomOffset(
      final OffsetDateTime parentEventTime,
      final RandomSerialNumberGenerator randomDateTimeGenerator) {
    // Using from and to value generate a random number using Mersenne twister
    final long offsetValue =
        randomDateTimeGenerator.generateIntInRange(customOffsetFrom, customOffsetTo);

    // Append the Minutes/Hours/Days to the parent event eventTime based on the custom offset type
    // value
    return switch (customOffsetType.toLowerCase()) {
      ...
      default ->
          throw new IllegalArgumentException(
              "Unsupported customOffsetType for eventTime generation : " + customOffsetType);
    };
  }
}
