...
public class RandomValueGenerator {
  ...
  // Method to generate the Mersenne Twister instance for each of the RandomGenerators to get
  // subsequent numbers when used multiple times
  public static void generateInstance(final List<RandomGenerators> randomGenerators) {
    if (randomGenerators != null) {
      randomGenerators.forEach(
          r -> {
            ...
            final TriangularDistribution triangularDistribution =
                new TriangularDistribution(
                    random, r.getMinValue(), r.getMeanValue(), r.getMaxValue());
