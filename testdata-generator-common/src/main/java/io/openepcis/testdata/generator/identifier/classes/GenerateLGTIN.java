...
public class GenerateLGTIN extends GenerateQuantity {
  ...
  private List<QuantityList> generateIdentifiers(
    ...
    try {
      ...
      final String prefix =
          syntax == IdentifierVocabularyType.URN ? LGTIN_URN_PART : dlURL + LGTIN_URI_PART;
      ...
      final String modifiedSgtin =
          syntax == IdentifierVocabularyType.URN
              ? CompanyPrefixFormatter.gcpFormatterWithReplace(lgtin, gcpLength).toString()
              : lgtin;
      ...
      if (SerialTypeChecker.isRangeType(serialType, count, rangeFrom)) {
        ...
        final List<String> randomSerialNumbers =
            randomSerialNumberGenerator.randomGenerator(RandomizationType.NUMERIC, 1, 20, count);
      ...
      throw new TestDataGeneratorException(
          "Exception occurred during generation of LGTIN class identifiers : " + ex.getMessage(),
          ex);
    }
  }
  ...
  private QuantityList quantityUrnCreator(final String epcClass) {
    ...
    if (!StringUtils.isBlank(quantityType)) {
      ...
      quantityFormatted.setUom(
          quantityType != null && quantityType.equals("Variable Measure Quantity") ? uom : null);
    }
