...
/**
 * Class to generate the random numbers using the Mersenne Twister (MT) algorithm where if the seed
 * is provided then Random numbers are generated based on the seed. If no seed is provided then
 * truly random numbers are generated.
 */
public class RandomSerialNumberGenerator {
  @Getter private final RandomGenerator random;
  ...
  private static final String ALPHA_NUMERIC_RANDOM_SET =
  ...
  /**
   * Method to generate random Alpha numeric serial identifiers based on provided max and min
   * length.
   *
   ...
   */
  public List<String> alphaNumericGenerator(
      final int minLength, final int maxLength, final int randomCount) {
    try {
      ...
      throw new TestDataGeneratorException(
          "Exception occurred during creation of Random Alphanumeric serial identifiers : "
              + ex.getMessage(),
          ex);
    }
  }
  ...
  public List<String> urlSafeGenerator(final int minLength, int maxLength, int randomCount) {
    try {
      ...
      throw new TestDataGeneratorException(
          "Exception occurred during creation of Random URL safe special identifiers : "
              + ex.getMessage(),
          ex);
    }
  }
   ...
   * @return returns the List<String> with integer formatted random numbers
   */
  public List<String> numericGenerator(
      final int minLength, final int maxLength, final int randomCount) {
    try {
      final List<String> randomList = new ArrayList<>();
      ...
      throw new TestDataGeneratorException(
          "Exception occurred during creation of Random Numeric serial identifiers : "
              + ex.getMessage(),
          ex);
    }
  }
  ...
  private String generateRandomID(
      final String characterSet, final int minLength, final int maxLength) {
  ...
  /**
   * Method to generate random number based on provided lower and upper bound using the Mersenne
   * twister
   *
