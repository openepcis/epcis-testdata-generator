...
export const mutations = {
  ...
  populateData (state, payload) {
    state.eventsData =
      payload.eventsData !== undefined ? payload.eventsData : []
    state.identifiersData =
      payload.identifiersData !== undefined ? payload.identifiersData : []
  },
...
export const actions = {
  buildRelations ({ context, commit, rootState, state, dispatch }, payload) {
    ...
    const eventNodeInfo = JSON.parse(
      JSON.stringify(
        rootState.modules.ConfigureNodeEventInfoStore.nodeEventInfoArray
      )
    )

    // Get all the information relevant to Connector modal
    const connectorInfo = JSON.parse(
      JSON.stringify(rootState.modules.ConnectorStore.connectorArray)
    )

    // Get all the information relevant to the Identifiers Node
    const identifiersNodeInfo = JSON.parse(
      JSON.stringify(
        rootState.modules.ConfigureIdentifiersInfoStore.identifiersArray
      )
    )
    ...
    for (const eventNode in state.diagram) {
      ...
      if (state.diagram[eventNode].name === 'Events') {
        const eventObj = {
          eventId: state.diagram[eventNode].id,
          eventType: state.diagram[eventNode].data.eventType,
          formData: {}
        }
        ...
        const eventFormData = eventNodeInfo.find(
          eventInfo =>
            parseInt(eventInfo.eventId) ===
            parseInt(state.diagram[eventNode].data.ID)
        )

        // Check if user has provided the values by filling the form
        if (
          eventFormData !== undefined &&
          eventFormData.eventInfo !== undefined
        ) {
          // Add the information to the allEventsInfoArray
          eventObj.formData[eventFormData.eventInfo.eventType] =
            eventFormData.eventInfo
        }
    ...
    for (const identifierNode in state.diagram) {
      ...
      if (
        state.diagram[identifierNode].name === 'Identifiers' ||
        state.diagram[identifierNode].name === 'ParentIdentifiers'
      ) {
        // Get the connectorInfo associated with the IdentifiersNode
        const identifiersConnections =
          state.diagram[identifierNode].outputs.output_1.connections

        // Loop over connections and identify the
        for (const connect in identifiersConnections) {
          const targetConnectId =
            state.diagram[identifierNode].outputs.output_1.connections[connect]
              .node
          const connectionInfo = connectorInfo.find(
            connect =>
              parseInt(connect.source) ===
                parseInt(state.diagram[identifierNode].id) &&
              parseInt(connect.target) === parseInt(targetConnectId)
          )

          // Get the eventNode information from allEventsInfoArray based on the targetConnectId
          const eventFormDataInfo = allEventsInfoArray.find(
            node => parseInt(node.eventId) === parseInt(targetConnectId)
          )
          ...
          if (eventFormDataInfo !== undefined) {
            ...
            if (
              state.diagram[identifierNode].name === 'Identifiers' &&
              Object.keys(eventFormDataInfo.formData)[0] !== undefined
            ) {
              const identifierObj = {
              ...
              if (
                (Object.keys(eventFormDataInfo.formData)[0] ===
                  'TransformationEvent' &&
                  identifiersConnections[connect].output === 'input_1') ||
                Object.keys(eventFormDataInfo.formData)[0] !==
                  'TransformationEvent'
              ) {
                ...
                eventFormDataInfo.formData[
                  Object.keys(eventFormDataInfo.formData)[0]
                ].referencedIdentifier.push(identifierObj)
              } else if (
                Object.keys(eventFormDataInfo.formData)[0] ===
                  'TransformationEvent' &&
                identifiersConnections[connect].output === 'input_2'
              ) {
                // If the eventType is TransformationEvent and if the identifiers node is connected to input_2 then treat it as input EPC/Quantity
                eventFormDataInfo.formData[
                  Object.keys(eventFormDataInfo.formData)[0]
                ].outputReferencedIdentifier.push(identifierObj)
              }
            }
            ...
            if (state.diagram[identifierNode].name === 'ParentIdentifiers') {
              ...
              for (const parentCon in inputConnections) {
                ...
                for (const con in connections) {
                  ...
                  const parentEventNode = allEventsInfoArray.find(
                    node =>
                      parseInt(node.eventId) === parseInt(connections[con].node)
                  )

                  if (
                    parentEventNode !== undefined &&
                    parentEventNode.formData !== undefined
                  ) {
                    parentEventCount =
                      parentEventNode.formData[
                        Object.keys(parentEventNode.formData)[0]
                      ] !== undefined
                        ? parseInt(parentEventCount) +
                          parseInt(
                            parentEventNode.formData[
                              Object.keys(parentEventNode.formData)[0]
                            ].eventCount
              ...
              parentEventCount =
                parseInt(parentEventCount) !== 0
                  ? parseInt(parentEventCount)
                  : 1

              if (
                eventData !== undefined &&
                (eventData.eventType === 'AggregationEvent' ||
                  eventData.eventType === 'TransactionEvent' ||
                  eventData.eventType === 'AssociationEvent')
              ) {
                const parentObj = {
                  identifierId: state.diagram[identifierNode].id,
                  parentCount: parseInt(eventData.eventCount)
                }
    ...
    for (const eventNode in state.diagram) {
      ...
      if (state.diagram[eventNode].name === 'Events') {
        ...
        const parentNodeInfo = allEventsInfoArray.find(
          node =>
            parseInt(node.eventId) === parseInt(state.diagram[eventNode].id)
        )

        // Get all output connections associated with the Events node to build the Parent-Child relationship
        const outputEventNodeConnections =
          state.diagram[eventNode].outputs.output_1.connections
        ...
        for (const connection in outputEventNodeConnections) {
          ...
          const childNodeInfo = allEventsInfoArray.find(
            node =>
              parseInt(node.eventId) ===
              parseInt(outputEventNodeConnections[connection].node)
          )
          ...
          if (childNodeInfo !== undefined) {
            ...
            const connectionInfo = connectorInfo.find(
              connect =>
                parseInt(connect.source) ===
                  parseInt(state.diagram[eventNode].id) &&
                parseInt(connect.target) === parseInt(childNodeInfo.eventId)
            )
            ...
            if (parentNodeInfo.formData !== undefined) {
              if (
                parentNodeInfo.eventType === 'TransformationEvent' &&
                Object.keys(parentNodeInfo.formData)[0] !== undefined
              ) {
                // For TransformationEvent obtain the identifiers from outputReferencedIdentifier
                parentReferenceIdentifiers =
                  parentNodeInfo.formData[
                    Object.keys(parentNodeInfo.formData)[0]
              ...
              parentReferenceIdentifiers = parentReferenceIdentifiers.map(
                function (identifier) {
                  const refIdObj = {
              ...
              parentReferenceIdentifiers = parentReferenceIdentifiers.filter(
                (tag, index, array) =>
                  array.findIndex(
                    t => t.parentNodeId === tag.parentNodeId
                  ) === index
              )
            }
            ...
            if (Object.keys(childNodeInfo.formData)[0] !== undefined) {
              ...
              childNodeInfo.formData[
                Object.keys(childNodeInfo.formData)[0]
              ].referencedIdentifier = childNodeInfo.formData[
                Object.keys(childNodeInfo.formData)[0]
              ].referencedIdentifier.concat(parentReferenceIdentifiers)
            }
          }
        }
      }
    ...
    for (const eventInfo in allEventsInfoArray) {
      ...
      const childFormData =
        allEventsInfoArray[eventInfo].formData[
          Object.keys(allEventsInfoArray[eventInfo].formData)[0]
        ]
      const childEventRefId =
        childFormData !== undefined ? childFormData.referencedIdentifier : []
      const childEventCount =
        childFormData !== undefined ? childFormData.eventCount : 0
      ...
      for (const childRef in childEventRefId) {
        if (
          childEventRefId[childRef].parentNodeId !== undefined &&
          childEventRefId[childRef].parentNodeId !== 0
        ) {
          childEventTotalEpcCount =
            childEventTotalEpcCount +
            parseInt(childEventCount) *
              parseInt(childEventRefId[childRef].epcCount)
          childEventTotalClassCount =
            childEventTotalClassCount +
            parseInt(childEventCount) *
              parseInt(childEventRefId[childRef].classCount)
          childEventInheritParentCount =
            childEventInheritParentCount +
            parseInt(childEventCount) *
              parseInt(childEventRefId[childRef].inheritParentCount)
      ...
      if (parentNodeIds.length > 0) {
        ...
        for (const parentNodeId in parentNodeIds) {
          const parentNodeInfo = allEventsInfoArray.find(
            node =>
              parseInt(node.eventId) === parseInt(parentNodeIds[parentNodeId])
          )
          ...
          for (const parentRef in parentEventRefId) {
            if (parentEventRefId[parentRef].epcCount !== undefined) {
              parentEventTotalEpcCount =
                parentEventTotalEpcCount +
                parseInt(parentEventRefId[parentRef].epcCount)
              parentEventTotalClassCount =
                parentEventTotalClassCount +
                parseInt(parentEventRefId[parentRef].classCount)
            }
          }
        }
        ...
        if (
          parentEventTotalEpcCount + parentEventInheritParentCount <
          childEventTotalEpcCount
        ) {
          const epcErrObj = {
            type: 'Instance Identifiers',
            eventId: allEventsInfoArray[eventInfo].eventId,
            eventType: allEventsInfoArray[eventInfo].eventType,
            eventCount: childFormData.eventCount,
            bizStep: childFormData.businessStep,
            disposition: childFormData.disposition,
            parentCount: parentEventTotalEpcCount,
            childCount: childEventTotalEpcCount,
            message:
              'Child node/event is inheriting more EPC identifiers than available in Parent node/event.'
          }
        ...
        if (parentEventTotalClassCount < childEventTotalClassCount) {
          const classErrObj = {
            ...
            bizStep: childFormData.businessStep,
            disposition: childFormData.disposition,
            parentCount: parentEventTotalClassCount,
            childCount: childEventTotalClassCount,
            message:
              'Child node/event is inheriting more Class identifiers than available in Parent node/event.'
          }
        ...
        if (parentEventInheritParentCount < childEventInheritParentCount) {
          const parentErrObj = {
            type: 'Parent Identifiers',
            eventId: allEventsInfoArray[eventInfo].eventId,
            eventType: allEventsInfoArray[eventInfo].eventType,
            eventCount: childFormData.eventCount,
            bizStep: childFormData.businessStep,
            disposition: childFormData.disposition,
            parentCount: parentEventInheritParentCount,
            childCount: childEventInheritParentCount,
            message:
              'Child node/event is inheriting more Parent Identifiers than available in Parent node/event.'
          }
    ...
    commit('populateData', {
      eventsData: allEventsInfoArray,
      identifiersData: identifiersNodeInfo
    })
  }
}
