...
export const mutations = {
  jsonPreparation (state, { rootState, payload }) {
    ...
    const events = []
    let eventsObj = { nodeId: 1, eventType: rootState.modules.DesignTestDataStore.eventType }
    ...
    if (payload.formData.eventID) {
      eventFormData.eventIdType = payload.formData.eventIdType !== undefined ? payload.formData.eventIdType : 'UUID'

      // For Hash Algorithm set the hashAlgorithmType
      if (payload.formData.eventIdType !== undefined && payload.formData.eventIdType === 'HashId') {
        eventFormData.hashAlgorithm = payload.formData.hashAlgorithm !== undefined ? payload.formData.hashAlgorithm : 'sha-256'
      }
    }
    ...
    if (payload.formData.eventTimeSelector === 'SpecificTime') {
      eventTime.specificTime = payload.formData.eventTime.specificTime + utcOffset
    } else {
    ...
    if (payload.formData.readpointselector != null && payload.formData.readpointselector !== 'null') {
    ...
    if (payload.formData.businesslocationselector != null && payload.formData.businesslocationselector !== 'null') {
    ...
    if (!payload.formData.ordinaryEvent) {
      ...
      if (payload.formData.error.ErrorDeclarationTimeSelector === 'SpecificTime') {
        declarationTime.specificTime = payload.formData.error.ErrorDeclarationTime + utcOffset
      } else {
        declarationTime.fromTime = payload.formData.error.ErrorDeclarationTimeFrom + utcOffset
        declarationTime.toTime = payload.formData.error.ErrorDeclarationTimeTo + utcOffset
      }
      ...
      if (payload.formData.ErrorReasonType !== 'Other') {
        if (payload.formData.error.ErrorReasonType !== null) {
          errorDeclaration.declarationReason = payload.formData.error.ErrorReasonType
        }
      } else {
        errorDeclaration.declarationReason = payload.formData.error.ErrorReasonOther
      }
      ...
      if (rootState.modules.ExtensionDataStore.errorExtensions.length > 0) {
        errorDeclaration.extensions = rootState.modules.ExtensionDataStore.errorExtensions
      }
    ...
    if (rootState.modules.SensorElementsStore.sensorElementList !== undefined && rootState.modules.SensorElementsStore.sensorElementList.length > 0) {
      const sensorElementList = []

      for (const sensorItem of rootState.modules.SensorElementsStore.sensorElementList) {
        ...
        const sensorObj = { ID: sensorItem.ID, sensorMetadata: sensorMetaData, sensorReport: sensorReportArray }
    ...
    if (rootState.modules.DesignTestDataStore.eventType === 'ObjectEvent' || rootState.modules.DesignTestDataStore.eventType === 'TransformationEvent') {
    ...
    const classIdentifiersList = JSON.parse(JSON.stringify(rootState.modules.IdentifiersStore.classIdentifiersList))
    ...
    if (instanceIdentifiersList.length > 0) {
      ...
      for (const instanceEpc of instanceIdentifiersList) {
        const epc = { identifierId: instanceEpc.ID, epcCount: 0, classCount: 0 }
      ...
      for (const classIdentifiers of classIdentifiersList) {
        const quantity = { identifierId: classIdentifiers.ID, epcCount: 0, classCount: 0 }
    ...
    const parentIdentifiersList = JSON.parse(JSON.stringify(rootState.modules.IdentifiersStore.parentIdentifiersList))
    ...
    const outputInstanceIdentifiersList = JSON.parse(JSON.stringify(rootState.modules.IdentifiersStore.outputInstanceIdentifiersList))
    const OutputlassIdentifiersList = JSON.parse(JSON.stringify(rootState.modules.IdentifiersStore.outputclassIdentifiersList))

    if (outputInstanceIdentifiersList.length > 0) {
      // Loop though the identifiersList and add the information to the referencedIdentifiers
      for (const outputInstanceEpc of outputInstanceIdentifiersList) {
        const epc = { identifierId: outputInstanceEpc.ID, epcCount: 0, classCount: 0 }
      ...
      for (const outputClassIdentifier of OutputlassIdentifiersList) {
        const quantity = { identifierId: outputClassIdentifier.ID, epcCount: 0, classCount: 0 }
  ...
  identifiersData (state, { rootState, payload }) {
    ...
    const parentIdentifiersList = JSON.parse(JSON.stringify(rootState.modules.IdentifiersStore.parentIdentifiersList))
    const outputInstanceIdentifiersList = JSON.parse(JSON.stringify(rootState.modules.IdentifiersStore.outputInstanceIdentifiersList))
    const outputclassIdentifiersList = JSON.parse(JSON.stringify(rootState.modules.IdentifiersStore.outputclassIdentifiersList))
    ...
    if (instanceIdentifiersList.length > 0) {
      ...
      for (const instanceEPC in instanceIdentifiersList) {
        const identifiersObj = { identifierId: instanceIdentifiersList[instanceEPC].ID, objectIdentifierSyntax: rootState.modules.IdentifiersStore.identifierSyntax, instanceData: {}, classData: null, parentData: null }
        identifiersObj.instanceData[instanceIdentifiersList[instanceEPC].type] = instanceIdentifiersList[instanceEPC][Object.keys(instanceIdentifiersList[instanceEPC])[3]]
    ...
    if (classIdentifiersList.length > 0) {
      ...
      for (const quantityClass in classIdentifiersList) {
        ...
        if (inputIdentifiers[quantityClass] !== undefined) {
          ...
          inputIdentifiers[quantityClass].classData[classIdentifiersList[quantityClass].type] = classIdentifiersList[quantityClass][Object.keys(classIdentifiersList[quantityClass])[3]]
        } else {
          // If identifiers list does not contain the associated value then create a new object and add the new one
          const identifiersObj = { identifierId: classIdentifiersList[quantityClass].ID, objectIdentifierSyntax: rootState.modules.IdentifiersStore.identifierSyntax, instanceData: null, classData: {}, parentData: null }
          identifiersObj.classData[classIdentifiersList[quantityClass].type] = classIdentifiersList[quantityClass][Object.keys(classIdentifiersList[quantityClass])[3]]
          inputIdentifiers.push(identifiersObj)
        }
      }
    }
    ...
    if (parentIdentifiersList.length > 0) {
      ...
      for (const parentClass in parentIdentifiersList) {
        // If identifiers list does not contain the associated value then create a new object and add the new one
        const identifiersObj = { identifierId: parentIdentifiersList[parentClass].ID, objectIdentifierSyntax: rootState.modules.IdentifiersStore.identifierSyntax, instanceData: null, classData: null, parentData: {} }
        identifiersObj.parentData[parentIdentifiersList[parentClass].type] = parentIdentifiersList[parentClass][Object.keys(parentIdentifiersList[parentClass])[3]]
        inputIdentifiers.push(identifiersObj)
      }
    }
    ...
    if (outputInstanceIdentifiersList.length > 0) {
      ...
      for (const outputEPC in outputInstanceIdentifiersList) {
        const identifiersObj = { identifierId: outputInstanceIdentifiersList[outputEPC].ID, objectIdentifierSyntax: rootState.modules.IdentifiersStore.identifierSyntax, instanceData: {}, classData: null, parentData: null }
        identifiersObj.instanceData[outputInstanceIdentifiersList[outputEPC].type] = outputInstanceIdentifiersList[outputEPC][Object.keys(outputInstanceIdentifiersList[outputEPC])[3]]
    ...
    if (outputclassIdentifiersList.length > 0) {
      ...
      for (const outputClass in outputclassIdentifiersList) {
        ...
        if (outputIdentifiers[outputClass] !== undefined) {
          ...
          outputIdentifiers[outputClass].classData[outputclassIdentifiersList[outputClass].type] = outputclassIdentifiersList[outputClass][Object.keys(outputclassIdentifiersList[outputClass])[3]]
        } else {
          // If identifiers list does not contain the associated value then create a new object and add the new one
          const identifiersObj = { identifierId: outputclassIdentifiersList[outputClass].ID, objectIdentifierSyntax: rootState.modules.IdentifiersStore.identifierSyntax, instanceData: null, classData: {}, parentData: null }
          identifiersObj.classData[outputclassIdentifiersList[outputClass].type] = outputclassIdentifiersList[outputClass][Object.keys(outputclassIdentifiersList[outputClass])[3]]
          outputIdentifiers.push(identifiersObj)
        }
      }
    }
    ...
    if (referencedIdentifiers !== undefined && Object.keys(referencedIdentifiers).length !== 0 && identifiersList.length > 0) {
      ...
      for (const refId of referencedIdentifiers) {
        const identifierInfo = identifiersList.find(idObj => idObj.identifierId === refId.identifierId)
        ...
        if (identifierInfo !== undefined && identifierInfo.instanceData != null && identifierInfo.instanceData !== undefined && Object.keys(identifierInfo.instanceData).length !== 0) {
          if (identifierInfo.instanceData[Object.keys(identifierInfo.instanceData)].serialType === 'range') {
            refId.epcCount = parseInt(identifierInfo.instanceData[Object.keys(identifierInfo.instanceData)[0]].count)
          } else if (identifierInfo.instanceData[Object.keys(identifierInfo.instanceData)[0]].serialType === 'random') {
            refId.epcCount = parseInt(identifierInfo.instanceData[Object.keys(identifierInfo.instanceData)[0]].randomCount)
          } else if (identifierInfo.instanceData[Object.keys(identifierInfo.instanceData)[0]].serialType === 'none') {
            refId.epcCount = parseInt(1)
          } else if (Object.keys(identifierInfo.instanceData)[0] === 'manualURI') {
            if (identifierInfo.instanceData[Object.keys(identifierInfo.instanceData)[0]].manualUriType === 'dynamic') {
              refId.epcCount = parseInt(identifierInfo.instanceData[Object.keys(identifierInfo.instanceData)[0]].manualUriRangeTo - identifierInfo.instanceData[Object.keys(identifierInfo.instanceData)[0]].manualUriRangeFrom)
            } else if (identifierInfo.instanceData[Object.keys(identifierInfo.instanceData)[0]].manualUriType === 'static') {
              refId.epcCount = parseInt(1)
            }
          }
        }
        ...
        if (identifierInfo !== undefined && identifierInfo.classData != null && identifierInfo.classData !== undefined && Object.keys(identifierInfo.classData).length !== 0) {
          if (identifierInfo.classData[Object.keys(identifierInfo.classData)].serialType === 'range') {
            refId.classCount = parseInt(identifierInfo.classData[Object.keys(identifierInfo.classData)[0]].count)
          } else if (identifierInfo.classData[Object.keys(identifierInfo.classData)[0]].serialType === 'random') {
            refId.classCount = parseInt(identifierInfo.classData[Object.keys(identifierInfo.classData)[0]].randomCount)
          } else if (identifierInfo.classData[Object.keys(identifierInfo.classData)[0]].serialType === 'none') {
            refId.classCount = parseInt(1)
          } else if (identifierInfo.classData[Object.keys(identifierInfo.classData)[0]].classIdentifiersCount !== undefined) {
            refId.classCount = parseInt(identifierInfo.classData[Object.keys(identifierInfo.classData)[0]].classIdentifiersCount)
          } else if (Object.keys(identifierInfo.classData)[0] === 'manualURI') {
            if (identifierInfo.classData[Object.keys(identifierInfo.classData)[0]].manualUriType === 'dynamic') {
              refId.classCount = parseInt(identifierInfo.classData[Object.keys(identifierInfo.classData)[0]].manualUriRangeTo - identifierInfo.classData[Object.keys(identifierInfo.classData)[0]].manualUriRangeFrom)
            } else if (identifierInfo.classData[Object.keys(identifierInfo.classData)[0]].manualUriType === 'static') {
              refId.classCount = parseInt(1)
            }
          }
        }
    ...
    const parentReferencedIdentifier = eventsInfoList[0].parentReferencedIdentifier
    ...
    if (parentReferencedIdentifier != null && parentReferencedIdentifier !== undefined && Object.keys(parentReferencedIdentifier).length !== 0) {
      // Find the matching instance identifier based on reference identifier identifierId and identifiersList identifierId and get the instance data
      const identifierInfo = identifiersList.find(idObj => idObj.identifierId === parentReferencedIdentifier.identifierId)

      // If the parentData within eventInfo has values then add the parent count to parentReferencedIdentifier
      if (identifierInfo !== undefined && identifierInfo.parentData !== undefined && Object.keys(identifierInfo.parentData).length !== 0) {
        parentReferencedIdentifier.parentCount = parseInt(eventsInfoList[0].eventCount)
      }
    }

    const outputReferencedIdentifier = eventsInfoList[0].outputReferencedIdentifier
    ...
    if (outputReferencedIdentifier !== undefined && Object.keys(outputReferencedIdentifier).length !== 0 && identifiersList.length > 0) {
      // Find the matching instance identifier based on reference identifier identifierId and identifiersList identifierId and get the instance data
