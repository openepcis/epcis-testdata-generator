...
export const mutations = {
  ...
  eventsExtractor (state, { dispatch, rootState, payload }) {
    ...
    for (
      let eventCount = 1;
      eventCount <= state.eventsList.length;
      eventCount++
    ) {
      ...
      for (const eventProperty in eventInfo) {
        switch (eventProperty) {
          ...
          case 'type':
            ...
            if (
              eventInfo.sourceList === undefined ||
              eventInfo.sourceList.length === 0
            ) {
              eventObj.sources = {
                type: null,
                glnType: 'SGLN',
            ...
            if (
              eventInfo.destinationList === undefined ||
              eventInfo.destinationList.length === 0
            ) {
              eventObj.destinations = {
                type: null,
                glnType: 'SGLN',
                gcpLength: null
              }
            ...
            if (eventInfo.type === 'ObjectEvent') {
              ...
              eventInfo.type === 'AggregationEvent' ||
              eventInfo.type === 'AssociationEvent'
            ) {
          ...
          case 'eventTimeZoneOffset':
            if (eventObj.eventTime === undefined) {
              ...
              eventObj.eventTime.timeZoneOffset =
                eventInfo.eventTimeZoneOffset !== undefined
                  ? eventInfo.eventTimeZoneOffset
                  : '+02:00'
            }
            break
          ...
          case 'sourceList':
            ...
            eventObj.sources.OtherSourceURI1 = eventInfo.sourceList[0].type
              .split(':')
              .pop()
            eventObj.sources.OtherSourceURI2 = eventInfo.sourceList[0].source
              .split(':')
              .pop()
            break
          ...
          case 'destinationList':
            ...
            eventObj.destinations.OtherDestinationURI1 =
              eventInfo.destinationList[0].type.split(':').pop()
            eventObj.destinations.OtherDestinationURI2 =
              eventInfo.destinationList[0].destination.split(':').pop()
            break
          ...
          case 'errorDeclaration':
            {
              ...
              for (const errorProperty in eventInfo.errorDeclaration) {
                ...
                if (errorProperty === 'declarationTime') {
                  ...
                  let errorDeclarationTime = new Date(
                    eventInfo.errorDeclaration.declarationTime
                  )
                  errorDeclarationTime.setMinutes(
                    errorDeclarationTime.getMinutes() -
                      errorDeclarationTime.getTimezoneOffset()
                  )
                  errorDeclarationTime.setSeconds(
                    errorDeclarationTime.getSeconds(),
                    0
                  )
                  errorDeclarationTime = errorDeclarationTime
                    .toISOString()
                    .slice(0, -1)
                  eventObj.error.ErrorDeclarationTime = errorDeclarationTime
                } else if (errorProperty === 'reason') {
                  eventObj.error.ErrorReasonType =
                    eventInfo.errorDeclaration.reason !== undefined
                      ? eventInfo.errorDeclaration.reason
                        .split(':')
                        .pop()
                        .toUpperCase()
                      : null
                  ...
                  for (const correctiveID of eventInfo.errorDeclaration
                    .correctiveEventIDs) {
                    const correctiveIdObj = {
                      ID: correctiveEventIDsCount,
                      value: correctiveID
                    }
                    errorCorrectiveIdsList.push(correctiveIdObj)
                    correctiveEventIDsCount++
                  }
                  eventObj.error.errorCorrectiveIdsList =
                    errorCorrectiveIdsList
                } else {
                  // If none of the pre-defined properties match then assign the values to extension
                  eventObj.error.errorExtensions[errorProperty] =
                    eventInfo.errorDeclaration[errorProperty]
                }
              }

              // If the error extensions are present then format them according to the required format
              const errorExtensionCount = 0
              eventObj.error.errorExtensions = extensionsReader(
                context,
                eventObj.error.errorExtensions,
                'ErrorExtension',
                errorExtensionCount
          ...
          case 'persistentDisposition':
            {
              ...
              if (
                eventInfo.persistentDisposition.set !== undefined &&
                eventInfo.persistentDisposition.set.length > 0
              ) {
                for (const setItem of eventInfo.persistentDisposition.set) {
                  const pdObj = {
                    ID: persistentDispositionCount,
                    type: 'set',
                    value: setItem.toUpperCase()
                  }
              ...
              if (
                eventInfo.persistentDisposition.unset !== undefined &&
                eventInfo.persistentDisposition.unset.length > 0
              ) {
                for (const unsetItem of eventInfo.persistentDisposition.unset) {
                  const pdObj = {
                    ID: persistentDispositionCount,
                    type: 'unset',
                    value: unsetItem.toUpperCase()
                  }
                  persistentDispositionList.push(pdObj)
                  persistentDispositionCount++
                }
          ...
          case 'bizTransactionList':
            {
              ...
              for (const bizTransactionItem of eventInfo.bizTransactionList) {
                ...
                bizTransactionObj.type = bizTransactionItem.type
                  .split(':')
                  .pop()
                bizTransactionObj.bizTransaction =
                  bizTransactionItem.bizTransaction
              ...
              for (const sensorItem of eventInfo.sensorElementList) {
                ...
                if (sensorItem.sensorMetadata !== undefined) {
                  ...
                  if (sensorItem.sensorMetadata.time !== undefined) {
                    ...
                    sensorTime.setMinutes(
                      sensorTime.getMinutes() - sensorTime.getTimezoneOffset()
                    )
                  ...
                  if (sensorItem.sensorMetadata.startTime !== undefined) {
                    let sensorTime = new Date(
                      sensorItem.sensorMetadata.startTime
                    )
                    sensorTime.setMinutes(
                      sensorTime.getMinutes() - sensorTime.getTimezoneOffset()
                    )
                  ...
                  if (sensorItem.sensorMetadata.endTime !== undefined) {
                    let sensorTime = new Date(
                      sensorItem.sensorMetadata.endTime
                    )
                    sensorTime.setMinutes(
                      sensorTime.getMinutes() - sensorTime.getTimezoneOffset()
                    )
                ...
                if (
                  sensorItem.sensorReport !== undefined &&
                  sensorItem.sensorReport.length > 0
                ) {
          ...
          case 'ilmd':
            eventObj.ilmd = extensionsReader(
              context,
              eventInfo.ilmd,
              'ilmd',
              1
      ...
      eventObj.userExtensions = extensionsReader(
        context,
        eventObj.userExtensions,
        'userExtension',
        1
    ...
    const parameter = {
      eventInfoArray: generatedEventsList,
      finalNodeId: generatedEventsList.length
    }
    dispatch(
      'modules/ConfigureNodeEventInfoStore/addNodeEventInfoArray',
      parameter,
      { root: true }
    )
  }
...
function extensionsReader (context, extensions, type, extensionCount) {
  ...
  for (const key in extensions) {
    ...
    if (extensions[key] instanceof Object) {
      const extensionObj = {
        dataType: 'complex',
        namespace: contextNamespaceFinder(context, namespace),
        localName,
        text: '',
        complex: extensionsReader(
          context,
          extensions[key],
          type,
          extensionCount
