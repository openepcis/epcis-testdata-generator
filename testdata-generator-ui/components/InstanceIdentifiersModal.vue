...
<template>
  <b-modal
    ...
    size="lg"
    :visible="$store.state.modules.IdentifiersStore.instaceIdentifiersModal"
    @hide="cancel"
  >
    <b-form id="identifiersForm" @submit.prevent="submitIdentifiersData">
      ...
      <span class="form-inline verticleSpace">
        ...
        <div v-show="identifiersForm.serialType == 'range'" class="verticleSpace horizontalSpace">
          ...
          >
          <input
            v-show="!$store.state.modules.IdentifiersStore.parentFlag"
            v-model="identifiersForm.count"
            type="number"
            placeholder="Count"
            title="Enter the Count of identifiers per event"
            class="form-control"
            :required="identifiersForm.serialType == 'range' && !$store.state.modules.IdentifiersStore.parentFlag"
            style="width:120px;"
          >
        </div>

        <div v-show="identifiersForm.serialType == 'random'" class="verticleSpace horizontalSpace">
          <input
            v-show="!$store.state.modules.IdentifiersStore.parentFlag"
            v-model="identifiersForm.randomCount"
            type="number"
            class="form-control"
            min="1"
            placeholder="Number of Objects"
            title="Enter the Number of Objects"
            :required="identifiersForm.serialType == 'random' && !$store.state.modules.IdentifiersStore.parentFlag"
          >
        </div>
        ...
        <div v-if="gcpVisibility && $store.state.modules.IdentifiersStore.identifierSyntax === 'URN'">
          <b-form-select v-model="identifiersForm.gcpLength" :options="commonDropdownInfos.companyPrefixs" :required="gcpVisibility && $store.state.modules.IdentifiersStore.identifierSyntax === 'URN'" />
        </div>
      ...
      <div v-if="identifiersForm.identifierType == 'manualURI'" class="form-group">
        <div class="form-inline">
          ...
          <div v-if="identifiersForm.manualUriType == 'dynamic'" class="verticleSpace horizontalSpace">
            ...
            <input
              v-model="identifiersForm.manualUriRangeTo"
              type="number"
              min="1"
...
export default {
  ...
  mounted () {
    ...
    if (currentIdentifiersInfo !== undefined && currentIdentifiersInfo !== null && Object.keys(currentIdentifiersInfo).length !== 0) {
      ...
      this.instanceIdentifierSelection(identifierInfo.identifierType)
      this.identifiersForm = identifierInfo
    }
  },
  methods: {
    ...
    submitIdentifiersData (event) {
      ...
      this.identifiersForm.count = this.identifiersForm.count !== undefined ? parseInt(this.identifiersForm.count) : undefined
      this.identifiersForm.randomCount = this.identifiersForm.randomCount !== undefined ? parseInt(this.identifiersForm.randomCount) : undefined
      ...
      if (this.$store.state.modules.IdentifiersStore.identifiersFlagType === 'TransformationEventOutputEPC') {
        this.$store.commit('modules/IdentifiersStore/saveoutputInstanceIdentifiersInformation', { instanceData: this.identifiersForm })
      } else if (this.$store.state.modules.IdentifiersStore.parentFlag) {
        // Based on the randomisation type assign the eventCount value
        if (this.identifiersForm.serialType === 'range') {
          this.identifiersForm.count = (parseInt(this.identifiersForm.rangeFrom) + parseInt(this.$store.state.modules.IdentifiersStore.eventCount)).toString()
        } else if (this.identifiersForm.serialType === 'random') {
          this.identifiersForm.randomCount = parseInt(this.$store.state.modules.IdentifiersStore.eventCount).toString()
        }

        // Populate the parentID list
        this.$store.commit('modules/IdentifiersStore/saveParentIdentifiersInformation', { instanceData: this.identifiersForm })

        // After populating parent flag unset the parentFlag
        this.$store.commit('modules/IdentifiersStore/parentFlagToggle')
      } else {
        // Send the data to the store and store the informaiton
        this.$store.commit('modules/IdentifiersStore/saveInstanceIdentifiersInformation', { instanceData: this.identifiersForm })
      }

      // After saving the information hide the modal
      this.$store.commit('modules/IdentifiersStore/hideInstanceIdentifiersModal')
    },
