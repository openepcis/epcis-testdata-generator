  ...
  <div id="designTestData">
    <div class="wrapper">
      <div class="col" style="margin-top: 2%;">
        ...
        >
          <div class="node d-flex justify-content-center" :style="`background: ${n.color}`">
            <span>{{ n.name }}</span>
          </div>
          <pre />
        </div>
    ...
    <ConfigureInstanceIdentifiersModal v-if="$store.state.modules.ConfigureIdentifiersInfoStore.instanceIdentifiersModal" />
    <ConfigureClassIdentifiersModal v-if="$store.state.modules.ConfigureIdentifiersInfoStore.classIdentifiersModal" />
...
export default {
  ...
  async mounted () {
    ...
    this.$df.on('nodeCreated', function (nodeId) {
      if (vm.nodeType === 'Event') {
        const nodeObj = { eventId: vm.$store.state.modules.ConfigureNodeEventInfoStore.nodeCounter, eventType: vm.$df.getNodeFromId(nodeId).data.eventType }

        // Add the respective created node information in nodeStore
        vm.$store.commit('modules/ConfigureNodeEventInfoStore/populateNodeInfo', nodeObj)
      } else if (vm.nodeType === 'Identifier') {
        const identifierNodeObj = { identifiersId: vm.$store.state.modules.ConfigureNodeEventInfoStore.nodeCounter, identifierSyntax: 'URN' }

        // Add the respective created node information in nodeStore
        vm.$store.commit('modules/ConfigureIdentifiersInfoStore/populateIdentifiersInfo', identifierNodeObj)
      }
    })
    ...
    this.$df.on('nodeRemoved', function (nodeID) {
      if (vm.selectedNodeInfo.nodeType === 'Events') {
        ...
        vm.$store.commit('modules/ConfigureNodeEventInfoStore/removeNodeEventInfo', vm.selectedNodeInfo.nodeId)
      } else if (vm.selectedNodeInfo.nodeType === 'Identifiers' || vm.selectedNodeInfo.nodeType === 'ParentIdentifiers') {
        // Remove the respective node  from Identifiers Node
        vm.$store.commit('modules/ConfigureIdentifiersInfoStore/removeIdentifiersInfo', vm.selectedNodeInfo.nodeId)
      }
    })
    ...
    this.$df.on('connectionCreated', function (connectObj) {
      ...
      const connect = connectorArray.find(con => parseInt(con.source) === parseInt(connectObj.output_id) && parseInt(con.target) === parseInt(connectObj.input_id))

      // Cancel the connection from same Source Node to same Target Node
      if (connect !== undefined) {
        alert('Connection already exists for the Nodes : ' + 'Node : ' + connectObj.id_output + 'and Node : ' + connectObj.id_input)
        vm.$df.removeSingleConnection(connectObj.id_output, connectObj.id_input, connectObj.output_class, connectObj.input_class)
      } else {
        ...
        vm.$store.commit('modules/ConnectorStore/saveConnectorInfo', connectorObj)
      }
    })
    ...
    this.$df.on('connectionRemoved', function (connectObj) {
      ...
      vm.$store.commit('modules/ConnectorStore/populateCurrentConnector', connectObj)
      vm.$store.commit('modules/ConnectorStore/removeConnectorInfo', connectObj)
    })
    ...
    this.$df.on('click', function (e) {
      if (e.detail === 2 && e.target.classList[0] === 'main-path') {
        ...
        if (vm.$df.getNodeFromId(vm.connectObj.output_id).name !== 'ParentIdentifiers') {
        // After the population of the parent EPCs/Quantities show the modal with all information
          ...
          vm.$store.commit('modules/ConnectorStore/populateCurrentConnector', vm.connectObj)
        }
      }
    })
  ...
  methods: {
    ...
    addNodeToDrawFlow (name, posX, posY) {
      ...
      posX = posX * (this.$df.precanvas.clientWidth / (this.$df.precanvas.clientWidth * this.$df.zoom)) - this.$df.precanvas.getBoundingClientRect().x * (this.$df.precanvas.clientWidth / (this.$df.precanvas.clientWidth * this.$df.zoom))
      posY = posY * (this.$df.precanvas.clientHeight / (this.$df.precanvas.clientHeight * this.$df.zoom)) - this.$df.precanvas.getBoundingClientRect().y * (this.$df.precanvas.clientHeight / (this.$df.precanvas.clientHeight * this.$df.zoom))

      const nodeSelected = this.listNodes.find(ele => ele.name === this.dragEventType)
      this.nodeType = nodeSelected.type

      // If type is Identifiers then add the node and increase identifierCode
      this.$df.addNode(name, nodeSelected.input, nodeSelected.output, posX, posY, nodeSelected.name, { ID: this.$store.state.modules.ConfigureNodeEventInfoStore.nodeCounter, eventType: this.dragEventType }, name, 'vue')

      // Increment the value of the nodeCounter in store after adding the node to drawflow canvas for unique node value
      this.$store.commit('modules/ConfigureNodeEventInfoStore/incrementNodeCounter')
    },
    ...
    exportDesignInfo () {
      ...
      const eventNodeInfo = JSON.parse(JSON.stringify(this.$store.state.modules.ConfigureNodeEventInfoStore.nodeEventInfoArray))

      // Get all the information relevant to the Identifiers Node in diagram
      const identifiersNodeInfo = JSON.parse(JSON.stringify(this.$store.state.modules.ConfigureIdentifiersInfoStore.identifiersArray))

      // Get all the information relevant to Connectors in diagram
      const connectorsInfo = JSON.parse(JSON.stringify(this.$store.state.modules.ConnectorStore.connectorArray))

      // Get the drawflow diagram information
      const drawflowInfo = this.$df.export()

      // Build a single JSON with all the above information
      const diagramExportData = { eventNodeInfo, identifiersNodeInfo, connectorsInfo, drawflowInfo }

      const DateTime = new Date().toISOString().replace('Z', '').replace('T', '')
      const textFileAsBlob = new Blob([JSON.stringify(diagramExportData, null, 4)], { type: 'text/json' })
      const downloadLink = document.createElement('a')
      downloadLink.download = 'Test_Data_Generator_Design_' + DateTime + '.json'
    ...
    importEventsList (event) {
      ...
      this.$store.commit('modules/ConfigureImportEventsListStore/showEventsListModal')
    },
    ...
    designSupplychainFromEvents () {
      ...
      const eventsList = JSON.parse(JSON.stringify(this.$store.state.modules.ConfigureNodeEventInfoStore.nodeEventInfoArray))
      let newPosX = 0
      ...
      for (const event of eventsList) {
        ...
        let posX = x * (this.$df.precanvas.clientWidth / (this.$df.precanvas.clientWidth * this.$df.zoom)) - (this.$df.precanvas.getBoundingClientRect().x * (this.$df.precanvas.clientWidth / (this.$df.precanvas.clientWidth * this.$df.zoom))) - 97
        let posY = y * (this.$df.precanvas.clientHeight / (this.$df.precanvas.clientHeight * this.$df.zoom)) - (this.$df.precanvas.getBoundingClientRect().y * (this.$df.precanvas.clientHeight / (this.$df.precanvas.clientHeight * this.$df.zoom))) - 37

        posX = posX + newPosX
        posY = posY - 200
        newPosX = newPosX + 250

        const nodeSelected = this.listNodes.find(ele => ele.name === event.eventType)
        this.$df.addNode('Events', nodeSelected.input, nodeSelected.output, posX, posY, nodeSelected.name, { ID: event.eventId, eventType: event.eventType }, 'Events', 'vue')
      }
    },
